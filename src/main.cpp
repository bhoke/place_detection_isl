...
void PlaceDetector::processImage()
{
  if(!currentImage.empty())
  {
    ...
    else
    {
      ...
      currentBasePoint.invariants = totalInvariants / norm_factor;
      std::cout << "current/norm " << currentBasePoint.invariants.at<float>(0,0) << std::endl;
      currentBasePoint.id = image_counter;
...
PlaceDetector::PlaceDetector()
{
  ...
  this->currentBasePoint.invariants = Mat();
  this->previousBasePoint.invariants = Mat();
}
